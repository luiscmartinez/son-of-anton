...
- **Rate-limit handling is per-phase.** A 403/429 during search returns `{ prs: [], rateLimitHit: true }`. A 403/429 during enrichment short-circuits the concurrent worker and returns the partial set of already-enriched PRs with `rateLimitHit: true`. The caller decides what to do with a partial result; this module never throws on rate-limit.
- **Post-closeout: forward-only (supersedes 90-day / 20-PR first-sync cap).** Outcome above records the original plan. Current code uses `resolveGithubMergedSince(since, now)` → `since ?? now`, `per_page=100` for all syncs, no `applyFirstSyncCap`. Agents should read [`phase-01-as-shipped-delta.md`](../../plans/phase-01-as-shipped-delta.md), not the Outcome cap bullets.
