...
1. `config.codogotchi?.enabled === false` guard in `writeGateEvent`: the check fires only on *exactly* `false`. What happens when `config.codogotchi` is `undefined` (optional field absent)? Does the write proceed correctly? Read `config.ts` to confirm the resolved-config shape.
