...
Setting `codogotchi.enabled: false` in `orchestrator.config.json` suppresses all event writes; no `.soa/` directory is created. See `docs/product/plans/codogotchi-alignment-draft.md` for the cross-repo alignment plan and the codogotchi repo's `docs/contracts/soa-event-feed.md` for the full NDJSON line schema.
